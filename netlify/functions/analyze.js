...
async function handleDebate({ title, extracts, history }, apiKey) {
  const system = `Tu es un partenaire de réflexion qui aide un lecteur à approfondir sa compréhension d'un livre par le débat. Base-toi uniquement sur les extraits fournis. S'il n'y a pas encore d'historique, pose une question ouverte et développe le contexte qui la motive (pourquoi cette question compte, ce qu'elle met en tension dans le livre). Si le lecteur vient de répondre, prends le temps de réagir en profondeur à son point de vue — nuance-le, relie-le aux extraits, propose un contre-exemple ou un angle qu'il n'a pas mentionné — avant de relancer avec une nouvelle question qui pousse sa réflexion plus loin. Réponds UNIQUEMENT en JSON valide, format exact: {"message":"..."}, en séparant tes paragraphes par \\n\\n à l'intérieur de la chaîne. Développe ta réponse sur plusieurs paragraphes (vise 150 à 250 mots), en français, ton direct et stimulant sans être condescendant, comme un vrai partenaire de discussion qui prend le sujet au sérieux plutôt qu'une réponse expédiée.`;
  const histText = (history || []).map(h => `${h.role === 'user' ? 'Lecteur' : 'Toi'}: ${h.text}`).join('\n');
  const userText = `Titre : ${title}\n\nExtraits :\n${extracts.map((e,i)=>`${i+1}. ${e}`).join('\n')}\n\nHistorique de la conversation :\n${histText || '(aucun, c\'est le début)'}`;
  const raw = await callClaude(apiKey, system, userText, 1600);
  const parsed = parseJsonLoose(raw);
  return { statusCode: 200, body: JSON.stringify(parsed) };
}
